...
Monitor your website's SSL/TLS certificate validity, expiry dates, and security configuration directly from your WordPress admin.
...
*   **Wildcard Verification:** Confirms if your certificate covers all subdomains (`*.yourdomain.com`).
*   **Subject Alternative Names (SANs):** Lists all specific domains and subdomains covered by the current certificate.
...
1.  Navigate to **Upsnap → Security Certificates** in your WordPress Admin.
2.  The **Status Card** will show your overall certificate health.
3.  The **Certificate Details** card provides deep technical info about the active certificate.
4.  **Domain Coverage:** Scroll down to see which specific domains are protected.
5.  **Chain Validation:** Click the **Show more** button to inspect the full certificate chain.

### Manual Refresh
If you have just installed a new certificate and want to verify it immediately, click the **Check Now** button in the top right. This triggers a fresh SSL/TLS audit via the UpSnap API.

## Troubleshooting
*   **"Certificate has expired!":** Your site is now showing security warnings to users. Log in to your hosting provider or certificate authority immediately to renew.
...
*   **Domain Mismatch:** Ensure the "Subject Alternative Names" list actually includes the URL you are using for your WordPress site.
